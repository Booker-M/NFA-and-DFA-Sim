...
function updateGroup(group, groups, nextGroups, description, newDescription, i) {
    ...
    if (group[0].includes(description.start)) { description.start = `q${i}`}
    if (description.acceptStates.some(state => group[0].includes(state))) { newDescription.acceptStates.push(`q${i}`)}
...
export function minimizeDFA(dfa: DFA): DFA {
    ...
    while (stack.length > 0) { //simplify/standardize state names
        let group = stack.pop()
        // console.log("BEFORE", "GROUPS: ", groups, "NEXT GROUPS: ", nextGroups)
        updateGroup(group, groups, nextGroups, description, newDescription, i)
        // console.log("AFTER", "GROUPS: ", groups, "NEXT GROUPS: ", nextGroups)
